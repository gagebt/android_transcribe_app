...
public final class TextFitter {
    ...
    public static Fit fit(String spoken, CharSequence before, CharSequence after,
        ...
        // --- fitter rule 3: the leading-capital change -----------------------------
        // REVERTIBLE UNIT. This one call and the applyLeadingCapital method below are
        // the whole of rule 3. Delete both and rules 1, 2 and 4 keep working exactly
        // as they do here; the model's own capital is then always kept.
        t = applyLeadingCapital(t, context);
        // --- end of fitter rule 3 ---------------------------------------------------
        ...
        // Step 4 — space after.
        //
        // The test is on A, not on the space-trimmed `a`: PRODUCT-DECISION section 2
        // states both "a starts with a letter" and "no space when A starts with
        // whitespace", which contradict each other, and its own worked row
        // (`I think we|` + ` go home.` -> "I think we should probably go home." with
        // "no double space") decides it. Testing `a` there would produce a double space.
        String suffix = "";
