...
//!   Speak for four minutes without pausing and it is one piece, and the wait
//!   is the wait the app has today — which is the floor the product owner set
//!   on 2026-09-19: *"Don't cap the text length."*
...
//!   next one instead. [`compact_for_inference`] removes only sustained clear
//!   quiet from the copy submitted to the model. Every retained sample is a
//!   sample the microphone delivered; no silence is padded or synthesised,
//!   which the model punishes. See [`ShortPiece`] and [`PieceFloor`].
...
/// Preparation for Parakeet, separate from segmentation so piece timing and
/// `pause_before` continue to describe the original microphone stream.
const INFERENCE_FRAME_SAMPLES: usize = 320; // 20 ms
const INFERENCE_CONTEXT_SAMPLES: usize = SAMPLE_RATE; // 1.0 s
...
/// app's own bundled `bench.wav` has a room tone of 0.0085 RMS — twice that
/// threshold — so the fixed gauge finds **no quiet stretch anywhere in that
/// clip** (measured on `ovh`, 2026-09-19: the longest run below 0.004 in the
/// whole 11 s clip is 0.06 s, while the clip plainly contains a 1.15 s pause).
/// Add any real background and it only gets worse. With no length cap, a gauge
...
pub enum SpeechGauge {
    ...
    /// judged speech. Each of those three words was bought with a measurement
    /// on `ovh`, 2026-09-19:
    ///
    ...
    Room {
        ...
        /// estimate near zero, every later frame then stood far above it, the
        /// gauge called whole clips speech, and not one pause was found —
        /// measured on `ovh`, 2026-09-19, where the 2 s and 3 s pause
        /// recordings each produced one piece instead of two. Keeping dead
...
pub enum ShortPiece {
    ...
    /// fragment may be a word, and the owner's words are not recoverable.
    Carry,
}
...
impl SegmenterConfig {
    /// Dictation with the owner-visible default split pause.
    ...
    pub fn dictation_with_split_seconds(split_seconds: f32) -> Self {
        ...
        SegmenterConfig {
            ...
            // model damages a word that sits close to a cut: measured on the
            // real model over 108 runs, a boundary with 60 ms of quiet after
            // the preceding word turned the name "Neuphonic" into "Nephonic",
            // "no phonic" and "nophonic", and at one setting deleted it, while
            // boundaries with generous quiet left every word intact. The split
            // threshold guarantees at least 1.5 s at a cut, so 0.5 s on each
            // side always exists and the two margins cannot meet. It costs
            // about 0.1 s of compute per boundary at the measured real-time
            // factor of 0.2 — cheap against a destroyed name.
            ...
            // below that the model starts losing the words, returning "Watch
            // your country." for "What your country can do for you." at 0.5 s
            // of context, and empty text for the shortest runs (measured, same
            //108 runs). It is held and merged into the next piece instead of
            // dropped, because it may be a word and the owner cannot say it
            // again. `flush` ignores this: a short last piece is what tapping
...
impl Segmenter {
    ...
    pub fn push(&mut self, block: &[f32]) -> Option<Piece> {
        ...
        if !self.has_speech {
            ...
            // of the owner waiting before he spoke through the model for
    ...
    fn finish(
        ...
        // a constant signal is off the distribution the model was trained on
        // and makes it repeat the last word — measured on the real model,
        // word-level corruption in 14 of 78 runs with exact zeros against 7 of
        // 78 with real room tone. The margins kept on both sides of each cut
...
///
/// The 10th-percentile frame level estimates the room in this piece. Frames
/// above 1.5 times that level are retained with 1.0 second of real recorded
/// context on both sides. Overlapping context ranges merge, so short pauses and
/// low-energy parts inside a word remain untouched. A uniformly quiet piece
/// returns empty and can skip inference. The original [`Piece`] stays intact,
/// so pause timing, retry audio and caption behavior do not change.
pub fn compact_for_inference(samples: &[f32]) -> Vec<f32> {
    ...
    let retained = ranges.iter().map(|(start, end)| end - start).sum();
...
mod tests {
    ...
    fn a_short_utterance_is_joined_to_the_next_piece_and_never_lost() {
        ...
        // not transcribed on its own — but it is the owner's word and he
        // cannot say it again, so it goes at the front of the next piece
    ...
    fn speech_without_a_pause_is_never_cut() {
        // The product owner's ruling of 2026-09-19: "Don't cap the text
        // length." Four minutes without a 1.5 s pause is one piece.
        let mut seg = dictation();
    ...
    fn unbroken_speech_over_background_noise_is_still_never_cut() {
        ...
        // the non-speech ones, invented a 2.18 s pause and so would have cut a
        // sentence in half (measured on ovh, 2026-09-19).
    ...
    fn the_room_window_stays_bounded_however_long_the_session_runs() {
        ...
        // callback drops the samples the owner is speaking into it.
    ...
    fn a_session_that_starts_in_the_middle_of_a_word_recovers() {
        ...
        // (measured: the cuts land at 8.2 s and 12.2 s, not at 4.2 s). That is
        // the safe direction — a missed boundary is one longer piece, which is
        // exactly today's behaviour — but it is a real limit and it is written
    ...
    #[test]
    fn inference_compaction_removes_only_the_middle_of_sustained_clear_quiet() {
        let audio = cat(&[
            quiet(3.0),
            speech(0.25),
            quiet(4.0),
        ...
        assert!(
            compacted.len() < 5 * SAMPLE_RATE,
            "holds {:.2} s",
            compacted.len() as f32 / SAMPLE_RATE as f32
        );
        assert!(compacted
            .windows((0.20 * SAMPLE_RATE as f32) as usize)
            .any(|window| { rms(window) >= TEST_SOUND_RMS }));
    }

    #[test]
    fn inference_compaction_keeps_an_immediate_stop_single_word_with_context() {
        let word = speech(0.28);
        let audio = cat(&[quiet(2.0), word.clone(), quiet(0.08)]);
        let compacted = compact_for_inference(&audio);
        assert!(compacted.len() >= word.len());
        assert!(compacted.windows(word.len()).any(|window| window == word));
        assert!(compacted.len() <= word.len() + INFERENCE_CONTEXT_SAMPLES * 2);
    }

    #[test]
    fn inference_compaction_preserves_quiet_speech() {
        let word = tone(0.35, 0.0008);
        let audio = cat(&[quiet(2.0), word.clone(), quiet(2.0)]);
        let compacted = compact_for_inference(&audio);
        assert!(compacted.windows(word.len()).any(|window| window == word));
    }

    #[test]
    fn inference_compaction_keeps_a_sparse_word_inside_long_quiet() {
        for level in [TEST_SOUND_RMS, 0.0008] {
            let word = tone(0.35, level);
            let audio = cat(&[quiet(35.0), word.clone(), quiet(35.0)]);
            let compacted = compact_for_inference(&audio);
            assert!(compacted.windows(word.len()).any(|window| window == word));
            assert!(
                compacted.len() <= (2.5 * SAMPLE_RATE as f32) as usize,
                "sparse word retained {:.2} s",
                compacted.len() as f32 / SAMPLE_RATE as f32
            );
        }
    }
