...
use crate::engine;
use crate::streaming::{
    compact_for_inference, CutCause, Segmenter, SegmenterConfig, DEFAULT_SPLIT_SECONDS,
    MAX_SPLIT_SECONDS, MIN_SPLIT_SECONDS,
};
...
pub const OUTCOME_REVIEW: i32 = 5;
...
struct Attempt {
    ...
    review_candidate: Mutex<Option<String>>,
}
...
impl Attempt {
    fn new(session_id: i64) -> Self {
        Self {
            ...
            review_candidate: Mutex::new(None),
        }
    }
...
fn notify_complete(env: &mut JNIEnv, obj: &JObject, attempt: &Attempt, outcome: i32, error: &str) {
    ...
    let text = attempt
        ...
        .unwrap()
        .clone()
        .unwrap_or_else(|| attempt.delivered_texts.lock().unwrap().join(" "));
...
fn full_review_candidate(delivered: &[String], combined_last_and_tail: &str) -> String {
    if combined_last_and_tail.is_empty() {
        return delivered.join(" ");
    }
    let earlier = delivered.len().saturating_sub(1);
    if earlier == 0 {
        return combined_last_and_tail.to_string();
    }
    let prefix = delivered[..earlier].join(" ");
    format!("{} {}", prefix, combined_last_and_tail)
}

fn settle_attempt(env: &mut JNIEnv, obj: &JObject, attempt: &Attempt) {
    attempt.processing.store(false, Ordering::SeqCst);
    if attempt.review_candidate.lock().unwrap().is_some() {
        notify_status(env, obj, attempt.session_id, "Review required");
        notify_complete(
            env,
            obj,
            attempt,
            OUTCOME_REVIEW,
            "final words need review; earlier words may already be in the field",
        );
        return;
    }
...
fn spawn_worker(
    ...
    std::thread::spawn(move || {
        ...
        while let Ok(job) = rx.recv() {
            ...
            if prepared_piece.is_empty() {
                // A stationary quiet remainder has no words and need not run
                // the model. Already delivered pieces remain successful.
                continue;
            }
...
pub fn retry_recording(mut env: JNIEnv, state: &mut VoiceSessionState, session_id: i64) -> bool {
    ...
    *attempt.failure.lock().unwrap() = None;
    *attempt.review_candidate.lock().unwrap() = None;
...
mod tests {
    ...
    #[test]
    fn final_review_candidate_keeps_all_earlier_accepted_pieces() {
        let delivered = vec![
            "Earlier first words.".to_string(),
            "Middle words.".to_string(),
            "Last context words.".to_string(),
        ];
        assert_eq!(
            full_review_candidate(&delivered, "Changed last context words. Final phrase."),
            "Earlier first words. Middle words. Changed last context words. Final phrase."
        );
    }

    #[test]
    fn final_review_candidate_does_not_duplicate_its_last_context_piece() {
        let delivered = vec!["Last context words.".to_string()];
        assert_eq!(
            full_review_candidate(&delivered, "Changed last context words. Final phrase."),
            "Changed last context words. Final phrase."
        );
    }

    #[test]
    fn empty_final_decode_keeps_every_already_recognized_piece() {
        let mut delivered = vec!["Earlier words.".to_string()];
        assert_eq!(full_review_candidate(&delivered, ""), "Earlier words.");
        delivered.push("Last recognized words.".to_string());
        assert_eq!(
            full_review_candidate(&delivered, ""),
            "Earlier words. Last recognized words."
        );
    }
    ...
    #[test]
    fn auto_stop_duration_reads_each_saved_value_and_defaults_invalid_input() {
        let dir =
